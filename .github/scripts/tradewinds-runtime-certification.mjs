...
function portableInputIssues(payload, artifacts) {
  ...
  fail(issues, local?.kind === 'world-parity-portable-report' && local.gitSha === '4b514373e521abac8920626f0e7e9a2c5dc03369', 'portable-local-build');
...
function verifyDraft(pack, root) {
  ...
  fail(issues, payload.binding?.gitSha === '4b51437' && payload.binding?.dirty === false, 'pack-binding');
