...
function portableInputIssues(payload, artifacts) {
  ...
  fail(issues, local?.kind === 'world-parity-portable-report' && local.gitSha === 'b29ad9cdfbe70dcb690c01c56191fb8d49205709', 'portable-local-build');
  ...
  for (const [index, run] of runs.entries()) {
    fail(issues, run?.kind === 'world-parity-normalized-vitest-run' && run?.success === true
      && run?.total === 127 && run?.passed === 127 && run?.failed === 0, `portable-vitest-shape:${index}`);
...
function verifyDraft(pack, root) {
  ...
  fail(issues, payload.binding?.gitSha === 'b29ad9cdfbe70dcb690c01c56191fb8d49205709'
